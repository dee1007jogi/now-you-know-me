...
window.onerror = function(msg, url, line, col, error) {
    ...
    div.style.cssText = "position:fixed; top:0; left:0; right:0; background:red; color:#0f172a; z-index:99999; padding:20px; font-family:monospace; font-size:14px; word-break:break-all;";
...
function renderModerationList(pendingSelfies) {
    ...
    if (pendingSelfies.length === 0) {
        list.innerHTML = `<div style="padding:40px; text-align:center; opacity:0.3; color:#0f172a;">No selfies pending review. 🕵️</div>`;
        return;
    }
    ...
    pendingSelfies.forEach(s => {
        ...
        item.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center;">
                ...
                <span style="font-size:0.8rem; background:${s.isCorrect ? '#10b981' : '#ef4444'}; color:#0f172a; padding:4px 10px; border-radius:8px; font-weight:800;">Guessed: ${s.guessedName} (${s.isCorrect ? 'Correct' : 'Wrong'})</span>
            </div>
...
function renderIncompleteGrid(players) {
    ...
    if (players.length === 0) {
        grid.innerHTML = '<div style="padding:40px; text-align:center; opacity:0.3; color:#0f172a;">No agents in lobby...</div>';
        return;
    }
    ...
    sortedPlayers.forEach(p => {
        ...
        row.innerHTML = `
            <div class="inc-p-info">
                ...
                <span style="font-weight:700; font-size:1rem; color:#0f172a;">${p.name || 'Anonymous Intelligence'}</span>
            </div>
...
function switchTab(activeBtn, activeContent) {
    ...
    if (!dbg) {
        ...
        dbg.style.cssText = "position:fixed; bottom:10px; right:10px; background:rgba(0,0,255,0.8); color:#0f172a; z-index:99999; padding:10px 20px; font-family:monospace; font-size:12px; border-radius:8px;";
...
window.showPlayerProfile = function(playerId) {
    ...
    if (p.answers) {
        ...
        Object.keys(p.answers).forEach(key => {
            ...
            div.innerHTML = `<strong style="color:var(--neon-cyan); display:block; font-size:0.7rem; text-transform:uppercase;">${label}</strong><span style="color:#0f172a; font-weight:600;">${val}</span>`;
    ...
    if (playerSelfies.length > 0) {
        playerSelfies.forEach(s => {
            ...
            div.innerHTML = `
                <img src="${s.photoUrl}" style="width:100%; height:100%; object-fit:cover;" />
                <div style="position:absolute; bottom:0; left:0; right:0; background:rgba(0,0,0,0.7); padding:4px; font-size:0.6rem; text-align:center; font-weight:800; color:#0f172a; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;" title="Guessed: ${s.guessedName}">
                    vs ${s.guessedName}
                </div>
            `;
        ...
        selfiesContainer.innerHTML = "<div style='grid-column: 1/-1; opacity:0.3; padding:10px; text-align:center; color:#0f172a;'>No selfies taken yet.</div>";
    }
...
window.renderLeaderboardList = function() {
    ...
    if (!appState || !appState.leaderboard || appState.leaderboard.length === 0) {
        container.innerHTML = `<div style="padding:40px; text-align:center; opacity:0.3; color:#0f172a;">Waiting for agents to join...</div>`;
        return;
    }
    ...
    if (filteredPlayers.length === 0) {
        container.innerHTML = `<div style="padding:40px; text-align:center; opacity:0.3; color:#0f172a;">No agents match this filter.</div>`;
        return;
    }
    ...
    filteredPlayers.forEach((p, i) => {
        ...
        const row = document.createElement("div");
        row.className = `leaderboard-row list-anim-item ${p.id === selectedPlayerId ? 'active' : ''} ${rankClass}`;
...
window.renderDossierDetail = function(playerId) {
    ...
    if (playerSelfies.length > 0) {
        ...
        selfiesHtml = `<div style="grid-column:1/-1; opacity:0.3; text-align:center; padding:15px; color:#0f172a;">No selfies captured yet.</div>`;
    }
...
window.renderSelfieStream = function() {
    ...
    if (!appState || !appState.selfies || appState.selfies.length === 0) {
        container.innerHTML = `<div style="padding:40px; text-align:center; opacity:0.3; color:#0f172a;">No selfies captured in this session yet. 📸</div>`;
    ...
    selfies.forEach((s, i) => {
        ...
        card.className = "stream-card list-anim-item";
