...
const startMarker = 'function loadAllMascots() {';
const startIndex = content.indexOf(startMarker);

// Find the last loadAllMascots(); after the function declaration
let endIndex = content.indexOf('loadAllMascots();', startIndex);
endIndex += 'loadAllMascots();'.length;

const newBlock = `async function loadAllMascots() {
                ...
                owl.model.position.set(5, 3, -40); // Deep outside, scaled down natively by perspective
                owl.model.rotation.set(0, 0, 0);

                // Glass break effects
                window.shatterGlass = function shatterGlass() {
                    // Play glass crash sound if we had one
                    ...
                    for(let i=0; i<40; i++) {
                        ...
                        gsap.to(shard.material, { opacity: 0, duration: 0.5, delay: 1 + Math.random(), onComplete: () => engine.scene.remove(shard) });
                    }
                    // Flash screen white for a frame
                ...
                await new Promise(resolve => {
                    ...
                    tl.to(owl.model.position, { z: -5, y: 1, duration: 0.6, ease: "power2.in" })
                      .call(window.shatterGlass)
                      // Swoop effect inside
                ...
                if (window.lipSyncManager) {
                    ...
                    // Start lipsync and wait for it either complete or timeout gracefully
...
fs.writeFileSync(file, content.substring(0, startIndex) + newBlock + content.substring(endIndex));
